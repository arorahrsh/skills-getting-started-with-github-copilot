document.addEventListener("DOMContentLoaded", () => {
  ...
  activitySelect.innerHTML = `<option value="">-- Select an activity --</option>`;
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Build participants HTML: show a bulleted list of participants with a remove icon
        const participantsHtml = details.participants && details.participants.length
          ? `<ul class="participants-list">${details.participants
              .map((p) => `<li>
                  <span class="participant-pill">${p}</span>
                  <button class="participant-remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}" aria-label="Remove ${p}">✕</button>
                </li>`)
              .join("")}</ul>`
        ...
        // Attach remove handlers for participant remove buttons inside this card
